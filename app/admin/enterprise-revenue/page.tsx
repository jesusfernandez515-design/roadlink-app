...
type RevenueSource = "corporate" | "university" | "airport" | "government" | "partnership" | "other";
...
export default function AdminEnterpriseRevenuePage() {
  ...
  const revenueAccounts = useMemo<RevenueAccount[]>(() => {
    const corporate: RevenueAccount[] = corporateAccounts.map((item) => ({
    ...
    const universities: RevenueAccount[] = universityPrograms.map((item) => ({
    ...
    const airports: RevenueAccount[] = airportPartnerships.map((item) => ({
      id: `airport-${item.id}`,
    ...
    const government: RevenueAccount[] = governmentContracts.map((item) => ({
    ...
    const partnerRevenue: RevenueAccount[] = partnerships.map((item) => ({
    ...
    const cleanManual: RevenueAccount[] = manualRevenue.map((item) => ({
      id: item.id,
      name: item.name || "Manual Revenue Account",
      source: item.source || "other",
      status: item.status || "forecast",
      monthlyRevenue: Number(item.monthlyRevenue || 0),
      annualRevenue: Number(item.annualRevenue || Number(item.monthlyRevenue || 0) * 12),
      clients: Number(item.clients || 0),
      activeUsers: Number(item.activeUsers || 0),
      monthlyRides: Number(item.monthlyRides || 0),
      notes: item.notes || "Manual enterprise revenue",
    }));
  ...
  }, [manualRevenue, corporateAccounts, universityPrograms, airportPartnerships, governmentContracts, partnerships]);
  ...
  const metrics = useMemo(() => {
    ...
    const mrr = active.reduce((total, item) => total + item.monthlyRevenue, 0);
    const forecastMrr = revenueAccounts
      .filter((item) => item.status !== "paused")
      .reduce((total, item) => total + item.monthlyRevenue, 0);

    const monthlyRidesTotal = revenueAccounts.reduce((total, item) => total + item.monthlyRides, 0);
    const activeUsersTotal = revenueAccounts.reduce((total, item) => total + item.activeUsers, 0);
    const clients = revenueAccounts.reduce((total, item) => total + item.clients, 0);
  ...
  function sourceRevenue(items: RevenueAccount[]) {
    return items.reduce((total, item) => total + item.monthlyRevenue, 0);
  }
    ...
    <main className="page">
              ...
              {revenueAccounts.map((item) => (
                <section key={item.id} className="accountCard">
                  <div className="cardTop">
                    <div>
                      <h3>{item.name}</h3>
                      <p>{sourceLabel(item.source)} • {item.notes}</p>
                    </div>
                  ...
                  <div className="infoGrid">
                    ...
                    <Info label="MRR" value={money(item.monthlyRevenue)} />
                    <Info label="ARR" value={money(item.annualRevenue)} />
                    <Info label="Clients" value={String(item.clients)} />
                    <Info label="Active Users" value={String(item.activeUsers)} />
                    <Info label="Monthly Rides" value={String(item.monthlyRides)} />
      ...
      <style>{`
        * { box-sizing: border-box; }
        .page { min-height: 100vh; color: white; padding: 24px; padding-bottom: 140px; font-family: Arial, sans-serif; background: radial-gradient(circle at top right, rgba(34,197,94,0.22), transparent 34%), radial-gradient(circle at bottom left, rgba(59,130,246,0.12), transparent 35%), linear-gradient(135deg, #020617, #030712, #0f172a); }
        .container { max-width: 1380px; margin: auto; }
        .topNav { display: flex; flex-wrap: wrap; gap: 12px; margin-bottom: 24px; }
        .miniButton { padding: 11px 18px; border-radius: 999px; background: rgba(255,255,255,0.04); border: 1px solid rgba(255,255,255,0.12); color: white; text-decoration: none; font-weight: 900; }
        .hero,.metric,.card,.accountCard,.sourceCard { background: rgba(8,13,25,0.92); border: 1px solid rgba(255,255,255,0.12); box-shadow: 0 24px 80px rgba(0,0,0,0.55); backdrop-filter: blur(16px); }
        .hero { border-radius: 34px; padding: 34px; margin-bottom: 22px; display: flex; justify-content: space-between; align-items: center; gap: 24px; }
        .eyebrow { margin: 0 0 10px; color: #22c55e; font-size: 13px; font-weight: 900; letter-spacing: 0.08em; text-transform: uppercase; }
        h1 { font-size: 58px; line-height: 1; margin: 0 0 16px; }
        h1 span, h2, .metricValue { color: #22c55e; }
        h2 { font-size: 30px; margin: 0 0 14px; }
        .subtitle,.empty p,.accountCard p,.sourceCard p { color: #a1a1aa; line-height: 1.5; }
        .scoreOrb { min-width: 104px; height: 104px; border-radius: 50%; background: rgba(34,197,94,0.12); border: 1px solid rgba(34,197,94,0.35); display: flex; align-items: center; justify-content: center; flex-direction: column; text-align: center; }
        .warningScore { background: rgba(239,68,68,0.12); border-color: rgba(239,68,68,0.35); }
        .scoreOrb strong { color: #22c55e; font-size: 32px; font-weight: 900; }
        ...
        .metricIcon { width: 42px; height: 42px; border-radius: 50%; background: rgba(34,197,94,0.13); display: flex; align-items: center; justify-content: center; font-size: 22px; margin-bottom: 12px; }
        .metricLabel { display: block; color: #a1a1aa; font-size: 12px; font-weight: 900; margin-bottom: 8px; }
        .metricValue { color: #22c55e; font-size: 22px; font-weight: 900; overflow-wrap: anywhere; }
        .sourceGrid { display: grid; grid-template-columns: repeat(3, 1fr); gap: 16px; margin-bottom: 24px; }
        .sourceCard { border-radius: 24px; padding: 20px; }
        .sourceIcon { width: 48px; height: 48px; border-radius: 50%; background: rgba(34,197,94,0.13); display: flex; align-items: center; justify-content: center; font-size: 24px; margin-bottom: 14px; }
        .sourceCard h3 { margin: 0 0 8px; font-size: 22px; }
        .sourceCard strong { display: block; color: #22c55e; font-size: 24px; margin-bottom: 6px; }
        .sourceCard p { margin: 0; }
        .card { border-radius: 30px; padding: 28px; margin-bottom: 24px; }
        .formGrid { display: grid; grid-template-columns: repeat(2, 1fr); gap: 12px; }
        input,select { width: 100%; padding: 15px; border-radius: 16px; border: 1px solid rgba(255,255,255,0.12); background: rgba(255,255,255,0.05); color: white; font-size: 16px; outline: none; }
        select option { color: black; }
        .saveButton { width: 100%; margin-top: 16px; padding: 16px; border-radius: 999px; border: none; color: white; font-weight: 900; cursor: pointer; background: linear-gradient(135deg, #22c55e, #16a34a); }
        .accountGrid { display: grid; gap: 16px; }
        .accountCard { border-radius: 24px; padding: 22px; box-shadow: none; }
        .cardTop { display: flex; justify-content: space-between; gap: 14px; align-items: flex-start; margin-bottom: 16px; }
        .accountCard h3 { margin: 0 0 6px; font-size: 22px; overflow-wrap: anywhere; }
        .accountCard p { margin: 0; overflow-wrap: anywhere; }
        .pill { padding: 8px 12px; border-radius: 999px; font-size: 12px; font-weight: 900; white-space: nowrap; color: #22c55e; background: rgba(34,197,94,0.12); border: 1px solid rgba(34,197,94,0.35); }
        .pill.paused { color: #fca5a5; background: rgba(239,68,68,0.12); border-color: rgba(239,68,68,0.35); }
        .infoGrid { display: grid; grid-template-columns: repeat(4, 1fr); gap: 12px; margin-bottom: 16px; }
        .infoBox { padding: 14px; border-radius: 16px; background: rgba(255,255,255,0.04); border: 1px solid rgba(255,255,255,0.1); }
        .infoBox span { display: block; color: #a1a1aa; font-size: 12px; font-weight: 900; margin-bottom: 6px; }
        .infoBox strong { display: block; overflow-wrap: anywhere; }
        .actions { display: flex; flex-wrap: wrap; gap: 10px; }
        .actions button { padding: 12px 16px; border-radius: 999px; border: none; font-weight: 900; color: white; cursor: pointer; background: rgba(59,130,246,0.14); border: 1px solid rgba(59,130,246,0.35); }
        .actions .dangerButton { background: rgba(239,68,68,0.14); border-color: rgba(239,68,68,0.35); color: #fca5a5; }
        .empty { padding: 24px; border-radius: 22px; background: rgba(255,255,255,0.04); border: 1px solid rgba(255,255,255,0.1); }
        .empty h3 { margin: 0 0 8px; font-size: 22px; }
        button:disabled { opacity: 0.6; cursor: not-allowed; }
        @media (max-width: 1180px) { .stats,.sourceGrid { grid-template-columns: repeat(2, 1fr); } .infoGrid { grid-template-columns: repeat(2, 1fr); } }
        @media (max-width: 720px) { .page { padding: 16px; padding-bottom: 140px; } .hero { flex-direction: column; align-items: flex-start; padding: 28px; } h1 { font-size: 44px; } .stats,.sourceGrid,.formGrid,.infoGrid { grid-template-columns: 1fr; } .cardTop { flex-direction: column; } }
      `}</style>
    </main>
  );
}
...
function SourceCard({ title, icon, value, count }: { title: string; icon: string; value: string; count: number }) {
  return (
